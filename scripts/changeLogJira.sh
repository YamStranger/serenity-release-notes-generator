...
export HARD_FILTERING_RELEASE=2016/01/28:20:02:+0000
...
mkdir -p tmp
cd tmp/
echo 'Serenity BDD Jira Plugin release notes generation'
git clone git@github.com:serenity-bdd/serenity-jira.git
cd serenity-jira/
git remote rename origin upstream
git remote add origin git@github.com:YamStranger/serenity-jira.git
git checkout master
git fetch upstream
git merge master upstream/master
git push origin -u master
git checkout -b release_notes_$RELEASE_DATE
github-changes --owner serenity-bdd -r serenity-jira -d pulls --token ${GITHUB_TOKEN} --file pulls.md --title 'Serenity BDD Jira Plugin change Log' --no-merges --verbose
github-changes --owner serenity-bdd -r serenity-jira -d commits --token ${GITHUB_TOKEN} --file commits.md --title 'Serenity BDD Jira Plugin change Log' --no-merges --verbose
echo '
...
{{#commits}}
{{#messageBody}}
* [commit:](https://github.com/serenity-bdd/serenity-jira/commit/{{hash}}) {{messageTitle}}
