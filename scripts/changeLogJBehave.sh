...
mkdir -p tmp
cd tmp/
echo 'Serenity BDD JBehave release notes generation'
git clone git@github.com:serenity-bdd/serenity-jbehave.git
cd serenity-jbehave/
git remote rename origin upstream
git remote add origin git@github.com:YamStranger/serenity-jbehave.git
git checkout master
git fetch upstream
git merge master upstream/master
git push origin -u master
git checkout -b release_notes_$RELEASE_DATE
github-changes --owner serenity-bdd -r serenity-jbehave -d pulls --token ${GITHUB_TOKEN} --file pulls.md --title 'Serenity BDD JBehave change Log' --no-merges --verbose
github-changes --owner serenity-bdd -r serenity-jbehave -d commits --token ${GITHUB_TOKEN} --file commits.md --title 'Serenity BDD JBehave change Log' --no-merges --verbose
echo '
