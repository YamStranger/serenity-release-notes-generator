...
export HARD_FILTERING_RELEASE=2016/02/05:18:30:+0000
...
mkdir -p tmp
cd tmp/
echo 'Serenity BDD Cucumber Plugin release notes generation'
git clone git@github.com:serenity-bdd/serenity-cucumber.git
cd serenity-cucumber/
git remote rename origin upstream
git remote add origin git@github.com:YamStranger/serenity-cucumber.git
git checkout master
git fetch upstream
git merge master upstream/master
git push origin -u master
git checkout -b release_notes_$RELEASE_DATE
github-changes --owner serenity-bdd -r serenity-cucumber -d pulls --token ${GITHUB_TOKEN} --file pulls.md --title 'Serenity BDD Cucumber Plugin change Log' --no-merges --verbose
github-changes --owner serenity-bdd -r serenity-cucumber -d commits --token ${GITHUB_TOKEN} --file commits.md --title 'Serenity BDD Cucumber Plugin change Log' --no-merges --verbose
echo '
...
{{#commits}}
{{#messageBody}}
* [commit:](https://github.com/serenity-bdd/serenity-cucumber/commit/{{hash}}) {{messageTitle}}
